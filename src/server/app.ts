...
import { Track, Playlist, ConnectedDevice, CloudSyncPayload } from '../types';
...
// Enable CORS for all incoming requests (crucial for Vercel & cross-origin previews)
...
function formatSongToTrack(s: any): Track {
  ...
  let cover =
    s.image || s.more_info?.image || 'https://images.unsplash.com/photo-1514525253161-7a46d19cd819?q=80&w=800';
...
// ----------------------------------------------------
// ROUTER DEFINITIONS (Supports both /api/* and root /* on Vercel)
// ----------------------------------------------------
const api = express.Router();

api.get('/health', (req, res) => {
  res.json({ status: 'ok', time: new Date().toISOString(), platform: process.env.VERCEL ? 'vercel' : 'standard' });
});

// Full Catalog Aggregate Endpoint
...
- triviaFact: An interesting artistic fact or studio recording insight`;
    ...
    res.json({
      ...
      audioMasterNote:
        'Mastered in high-resolution 24-bit audio to capture micro-dynamics, vocal breath, and sub-bass resonance without compression artifacts.',
      triviaFact: 'The producers blended analog tube gear with modern binaural 3D panners for realistic spatial depth.',
    });
  }
});

// Mount the API router at both '/api' (standard full-stack / Vite dev) AND at root '/' (Vercel serverless rewrites)
app.use('/api', api);
app.use(api);
